import numpy as np
import sys
import os

os.system("python $PYTHON_PATH/find_mol_nr.py 4")
A=np.array(np.loadtxt('input.txt'),dtype=int)
#os.system("rm input.txt")
print(A)
natoms_arr=A[3::2]//A[2::2]


def fix_bond(i,j):
    i1=int(i)
    if(i1==0):
        return "0"
    return "%d"%(i1+j)


fp_in  = open('fort.5','r')
fp_out = open('fort.5_2','w')
NX=int(sys.argv[1])
NY=int(sys.argv[2])
NZ=int(sys.argv[3])

lines=fp_in.readlines()

# number of atoms in fort.5
Natoms1=int(lines[-1].split()[0])
Nmols1=int(lines[4].split()[0])

# box size of fort.5
[LX,LY,LZ] = [float(i) for i in lines[1].split()]

fp_out.write('box:\n')
fp_out.write('%f'%(NX*LX))
fp_out.write(' %f'%(NY*LY))
fp_out.write(' %f\n'%(NZ*LZ))
fp_out.write('0.0\n')
fp_out.write('molecules_total_number:\n')
fp_out.write('%d\n'%(NX*NY*NZ*Nmols1))

molnr=0
atomnr=0
boxnr=0
lines=lines[5:]
mol_start=0
atom_start=0

# moltypenr




for natoms2 in natoms_arr:
    for ix in range(NX):
        for iy in range(NY):
            for iz in range(NZ):
                
                for l in lines:
            
                    ls=l.split()
                    if(len(ls)==2):
                        ls[1]="%d"%(int(ls[1])+mol_start)
                    if(len(ls)==1):
                        ls[0]="%d"%(int(ls[0]))
                        #Number of atoms in molecule
                        natoms=int(ls[0])
                    if(len(ls)>5):
                        # fix atom nr
                        ls[0]="%d"%(int(ls[0])+atom_start)
                 
                    # fix positions
                        ls[4]="%f"%(float(ls[4].replace('D','E')) +(ix)*LX)
                        ls[5]="%f"%(float(ls[5].replace('D','E')) +(iy)*LY)
                        ls[6]="%f"%(float(ls[6].replace('D','E')) +(iz)*LZ)

                    # fix bonds
                        ls[10:]=[fix_bond(i,atom_start) for i in ls[10:]]
                    l_out=' '.join(ls)
                    if(natoms2==natoms):
                        fp_out.write("%s\n"%(l_out))
                mol_start  = mol_start + Nmols1
                atom_start = atom_start + Natoms1

fp_out.close()
fp_in.close()


