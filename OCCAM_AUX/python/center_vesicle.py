...
while True:
    ...
    for i in range(N):
        ...
        if(str[0]=='C' or str[0]=='D' or str[0]=='T'):
            data2.append([float(str[1]),float(str[2]),float(str[3])])
            
    
    data2=np.array(data2)
    # for l in range(20):
    #     y= np.abs(data2[1:,:]-data2[0:-1,:])>0.5*np.array([LX,LY,LZ])
    #     if(sum(sum(y))==0):
    #         break
    #     if(sum(y[:,0])):
    #         data[:,0] += LX/20.0
    ...
    #         data[:,2] += LY/20.0
    #         data2[:,2] += LY/20.0
    #     data2 = data2 - np.array([LX,LY,LZ])*np.floor(data2/np.array([LX,LY,LZ]))
    #     data  = data  - np.array([LX,LY,LZ])*np.floor(data/np.array([LX,LY,LZ]))
                
    # data = data - np.array([LX,LY,LZ])*np.floor(data/np.array([LX,LY,LZ]))
    R_CM = np.mean(data2[:,:],0)
