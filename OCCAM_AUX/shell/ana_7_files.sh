...
function_name () { 
    fn=$1
    search_name=$2    
    name=$(python3 -c "print('${fn}-${search_name}'.replace('_','').replace(' ','').replace('.7','').upper())")
    folder=$(dirname ${filename})
    echo $folder
    rm -f ${name}_PLOT_DATA.dat
    echo -e "#\t$folder\t">${folder}/${name}_PLOT_DATA.dat  
#    echo "$search_name"
    cat ${folder}/$fn |  grep "$search_name"         | awk  ' {print $1"\t"$2}' >> ${folder}/${name}_PLOT_DATA.dat         
}

search_names=("vol" "    dens" "    press " "L_x" "L_z" "pressxx " "presszz " "etot_W" "  press_avg" "pressxx_avg" "presszz_avg")
for filename in $(find . | grep '.7' | grep -v 17); do
    echo $filename
