...
function_name () { 
    fn=$1
    search_name=$2    
    name=$(python -c "print('${search_name}'.replace('_','').replace(' ','').upper())")
...
# COMBINE ALL FILES
for sn in "${search_names[@]}";do    
    name1=$(python -c "print('${sn}'.replace('_','').replace(' ','').upper())")
    name2=$(python -c "print('${sn}'.replace('_','').replace(' ','').lower())")
    paste -d '\t' ../SIM_*/${name1}_PLOT_DATA.dat     >> ${name2}.dat     
    sort -n -k 1 -o ${name2}.dat     ${name2}.dat     
    python ${PYTHON_PATH}/combine_press.py ${name2}.dat      
done
cd ..
