...
\fontfamily{cmss}{\begin{tikzpicture}
  ...
  \draw[black,rounded corners=10,ultra thick] (a.south east)rectangle(boundingbox.north west);
        ...
        %External field
        \node[blue,label={above,blue}:{\Large$\nabla_x V_{\frac322}$}] at(7.5,-1){\Large$\gv\times$};
        \node[blue,label={below,blue}:{\Large$\nabla_x V_{\frac321}$}] at(7.5,-4){\Large$\gv\times$};
        \node[blue,label={left,blue,xshift=+0.3cm}:{\Large$\nabla_y V_{1\frac32}$}] at(6,-2.5){\Large$\gv\times$};
        \node[blue,label={right,blue,xshift=-0.25cm}:{\Large$\nabla_y V_{2\frac32}$}] at(9,-2.5){\Large$\gv\times$};
        \end{tikzpicture}};
